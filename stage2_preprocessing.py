...
def opencv_transform(pil_image):
    """
    Converts a raw PIL image from the dataset into a normalized PyTorch tensor.
    This function will be used as the transform in our DataLoader.
    """
    ...
    # cv2.resize takes (width, height) — note the order reversal vs NumPy (H, W)
    # For CIFAR-10 this is a no-op (already 32×32), but critical for Stage 6
    # when webcam frames arrive at 640×480 and must be resized before inference
    img = cv2.resize(img, TARGET_SIZE)               # shape: (32, 32, 3)

    # Step 3: RGB -> BGR -> RGB
    # OpenCV's native color order is BGR (historical camera convention).
    # We make this explicit now so in Stage 6, when cv2.VideoCapture gives us
    # BGR frames directly, we only change COLOR_RGB2BGR to a no-op here.
    # The round-trip is harmless for CIFAR-10 and keeps the pipeline consistent.
    img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)       # shape still: (32, 32, 3)

    # Step 4: Scale pixel values from 0–255 to 0.0–1.0
    img = img.astype(np.float32) / 255.0             # shape: (32, 32, 3), dtype: float32

    # Step 5: Normalize each channel using global mean and std
    # For every red pixel:   (value - 0.4914) / 0.2470
    # For every green pixel: (value - 0.4822) / 0.2435
    # For every blue pixel:  (value - 0.4465) / 0.2616
    ...
    # PyTorch CNNs expect [channels, height, width] — channels first.
